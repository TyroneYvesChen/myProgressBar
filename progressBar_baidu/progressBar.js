(function($){
    ...
    function fixDistance(e, dom, _this) {
        var bgToWindow = $(dom).find('.BMap_stdMpSliderMask').offset().top,
            topToWindow = $(dom).find('.BMap_stdMpZoom').offset().top;
        _this.bgTop = bgToWindow - topToWindow;
        _this.top = e.pageY - bgToWindow - _this.bgTop / 2;
        topVerify(_this);
    }
    ...
    function barMove(dom, _this, isMove) {
        var barHeight = _this.barHeight;
        topVerify(_this);
     	var bgBotTop = barHeight-_this.top;
